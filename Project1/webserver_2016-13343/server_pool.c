...
#define PORT 5000
#define NUM_THREAD 6 // number of threads in thread pool

char *ROOT;
void sendContentTypeHeader(int sock, char *file_path);
void respond(void *vargp);
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        // allocate new memory for sockfd integer. code before made to see memory beyond array allocation.
        ...
        **newsockfd = accept(sockfd, (struct sockaddr *)&cli_addr, &clilen);
        ...
        thpool_add_work(thpool, respond, (void *)*newsockfd);
    }
 ...
 *
 * Args
 * vargp: void* type for integer matching with the socket descriptor
 *
 * returns: void
 */
void respond(void *vargp)
{
    int *sockptr = (int *)vargp;
    int sock = *sockptr;
    int offset, bytes;
    char buffer[9000];
    bzero(buffer, 9000);
...
EXIT:
    ...
    close(sock);
    // need to free allocated memory for newsockfd
    free(sockptr);
}
